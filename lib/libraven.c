...
void DrawSpaceTheme(Font font, MusicMetadata *metadata)
{
  // Constants
  ...
  snprintf(infoText, sizeof(infoText),
           "Title: %s\nArtist: %s\nAlbum: %s\nSample Rate: %d Hz\nChannels: %d\nSample Size: "
           "%d-bit\nDuration: %.2f sec",
           title, artist, album, state->music.stream.sampleRate, state->music.stream.channels,
           state->music.stream.sampleSize, metadata->duration);
...
void easC_post_reload(easC_State *prev)
{
  ...
  if (IsMusicReady(state->music)) {
    ...
    state->overlay = LoadRenderTexture(screenWidth, screenHeight);
  }
  extract_metadata(selected_song, &state->metadata);
}
...
void easC_update()
{
    ...
    if (IsFileDropped())
    {
      if (IsMusicReady(state->music)) {
        ...
        if (droppedFiles.count > 0)
        {
          ...
          SetMusicVolume(state->music, currentVolume);
          extract_metadata(file_path, &state->metadata);
          AttachAudioStreamProcessor(state->music.stream, callback);
        }
    ...
    if (IsKeyPressed(KEY_F))
    {
      if (IsMusicReady(state->music)) {
        ...
        {
          UnloadMusicStream(state->music);
          state->music = LoadMusicStream(selected_song);
          PlayMusicStream(state->music);
          SetMusicVolume(state->music, currentVolume);
          extract_metadata(selected_song, &state->metadata);
          AttachAudioStreamProcessor(state->music.stream, callback);
    ...
    BeginTextureMode(state->overlay);
    DrawRectangle(0, 0, screenWidth, screenHeight, ColorAlpha(GRAY, 0.2f));
    EndTextureMode();
    DrawTextureRec(state->overlay.texture, (Rectangle){0, 0, screenWidth, -screenHeight}, (Vector2){0, 0},
                   WHITE);
    ...
    // Draw song details
    float totalDuration = GetMusicTimeLength(state->music);
    float currentTime   = GetMusicTimePlayed(state->music);

    char timeBuffer[100];
    snprintf(timeBuffer, sizeof(timeBuffer), "%.0f / %.0f sec", currentTime, totalDuration);
    ...
    const char* status = IsMusicStreamPlaying(state->music) ? "Playing" : "Paused";
    DrawTextEx(font, status, (Vector2){10, 10}, 20, 1,
               IsMusicStreamPlaying(state->music) ? GRUVBOX_GREEN : GRUVBOX_RED);
    ...
    if (showInfo)
    {
      DrawSpaceTheme(font, &state->metadata);
    }
    if (showHelp)
    {
...
void easC_init(const char* selected_song) {
  ...
  currentVolume = 0.8f;          // Volume control (initially set to full)
  lastVolume    = currentVolume; // Used for toggling mute/unmute state

  SetMusicVolume(state->music, currentVolume);
  PlayMusicStream(state->music);
  AttachAudioStreamProcessor(state->music.stream, callback);
  font    = LoadFontEx("resources/fonts/monogram.ttf", 24, NULL, 0);
  state->overlay = LoadRenderTexture(screenWidth, screenHeight);
}
